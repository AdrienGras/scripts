...
INSTALLED_VERSION=$(dbeaver --version 2>/dev/null | grep -Po 'Version \K[\d.]+' || echo "0")

if [ "$INSTALLED_VERSION" = "$VERSION" ]; then
    echo "DBeaver est déjà à jour ($VERSION)"
    exit 0
fi
...
# Cherche le fichier .deb pour amd64
DEB_URL=$(echo "$ASSETS_JSON" | grep '"browser_download_url"' | grep "amd64\.deb" | cut -d '"' -f 4 | head -1)
...
# Télécharge le .deb
TMP_DEB="/tmp/dbeaver_${VERSION}.deb"
echo "Téléchargement..."
...
echo "DBeaver $VERSION installé avec succès"
